...
def get_text_and_lrml(df, simplifications):
    ...
    if 'ir' in simplifications:
        ...
        if 'input_pred_train' in df.columns:
            print('Using Non-Overfitting Predictions for Training')
            text = df['input_pred_train']
        else:
            text = df['input_pred']
...
EXPERIMENT_NAMES = ['ir-consi', 'ir-consi-fixed-loss2']
...
if __name__ == '__main__':
    ...
    parser.add_argument('-es', '--eval-strategy', type=str, default='epoch',
                        help='Strategy for evaluation: epoch or steps?')
    parser.add_argument('--skip', type=int, default=0,
                        help='Strategy for evaluation: epoch or steps?')
    parser.add_argument('-pe', '--pred-epoch', type=int, default=3,
                        help='After how many epochs should the predictions be generated?')
                ...
                for train_type in ['ir', 'train-pred', 'train-oracle', 'train-oracle-pred']:
                    ...
                    for model_name1, model_path1 in model_paths_dict.items():
                        ...
                        exp_name = model_name1 + '_' + \
                            str(exp) + '_' + \
                            EXPERIMENT_NAMES[exp] + '_pe' + str(args.pred_epoch) + '_' + train_type + '_' + split + '_esp' + str(
                                args.early_stopping_patience) + '_e' + str(args.epochs) + '_' + str(seed)

                        if train_type == 'ir':
                            df['input'] = 'translate English to LegalRuleML: ' + \
                        ...
                        # Only for first training required
                            ...
                            df['label'] = df['lrml'].apply(
                                lambda x: get_original(x, [])).apply(tree_based_spacing)
                            df['input_no_sep'] = 'translate English to LegalRuleML: ' + get_file_names_from_df(
                                df) + ' ' + df['text']
                            df['input_pred'] = 'translate English to LegalRuleML: ' + get_file_names_from_df(
                                df) + ' ' + df['text'] + '<sep>' + all_predictions[:len(df)]
                            if train_predictions is not None:
                                df['input_pred_train'] = 'translate English to LegalRuleML: ' + get_file_names_from_df(
                                    df) + ' ' + df['text'] + '<sep>' + train_predictions[:len(df)]
                            df['input_oracle'] = 'translate English to LegalRuleML: ' + get_file_names_from_df(
                                df) + ' ' + df['text'] + '<sep>' + df['label']
                            metric_for_best_model = 'lrml_f_score'
                        ...
                        # pred_epoch decides if taking the best model to predict the IR or after how many epochs the predictions are generated
                        hyperparameters = dict(epochs=args.epochs, lr=lr, bs=bs, adafactor=False, weight_decay=1.000e-1, metric_for_best_model=metric_for_best_model,
                                               ...
                                               fp16=False, split=split, experiment=EXPERIMENTS[exp], exp_num=exp, calc_loss=bool(exp), pred_epoch=args.pred_epoch)
